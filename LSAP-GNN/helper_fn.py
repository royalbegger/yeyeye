...
'''
def avoid_coll(prednp, param_dict):
    # prednp: 软分配矩阵 [N, N]，值越大表示越可能匹配
    # param_dict: 包含 'N' 的字典
    # 返回: 硬分配索引 [N]，表示每个任务分配到的资源索引

    # 将得分转换为成本（匈牙利算法最小化成本，所以取负值）
    cost_matrix = -prednp  # 负值转换，因为 prednp 是得分，越大越好
    # 调用匈牙利算法，获取最优匹配
    row_ind, col_ind = linear_sum_assignment(cost_matrix)
    # row_ind 是任务索引，col_ind 是对应的资源索引
    return col_ind  # 返回 [N] 形状的硬分配索引
